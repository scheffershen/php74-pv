...
RUN if [ ${INSTALL_IMAGEMAGICK} = true ]; then \
    apt-get update && \
    apt-get install -y libmagickwand-dev imagemagick --fix-missing && \
...
RUN if [ ${INSTALL_PYTHON3} = true ]; then \
  apt-get -y install python3 python3-dev python3-tk python3-opencv build-essential ffmpeg libsm6 libxext6 libgl1-mesa-glx \
